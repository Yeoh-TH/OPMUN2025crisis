...
const locations: britTroops[] = [
  {key: "Singapore", location: { lat: 1.3521, lng: 103.8193 }},
];
...
function Editor(){
  ...
  const [coords, setCoords] = useState<{ imageX: number; imageY: number; mappedX: number; mappedY: number }>({ imageX: 0, imageY: 0, mappedX: 0, mappedY: 0 });
  ...
  const handleContextMenuUnit = (unitId: string) => (e: React.MouseEvent) => {
    ...
    if (mapOverlayRef.current) {
      ...
      // Compute unit center relative to the map overlay and relative to the image
  // const mapRect not needed since we only map to image coordinates
      const unitEl = document.getElementById(unitId);
      const imageEl = mapImageRef.current;
      if (unitEl) {
        const unitRect = unitEl.getBoundingClientRect();
  // (we only need image coordinates mapped to the requested system)

        // Compute coordinates relative to the actual displayed image
        let imageX = 0;
        let imageY = 0;
        let mappedX = 0;
        let mappedY = 0;
        if (imageEl) {
          ...
          const centerImageXDisplayed = Math.round(unitRect.left - imageRect.left + unitRect.width / 2);
          const centerImageYDisplayed = Math.round(unitRect.top - imageRect.top + unitRect.height / 2);

          // If natural size available, convert displayed pixels to image pixels
          const hasNatural = Boolean(imageEl.naturalWidth && imageEl.naturalHeight && imageRect.width > 0 && imageRect.height > 0);
          if (hasNatural) {
            const scaleX = imageEl.naturalWidth / imageRect.width;
            const scaleY = imageEl.naturalHeight / imageRect.height;
            imageX = Math.round(centerImageXDisplayed * scaleX);
            imageY = Math.round(centerImageYDisplayed * scaleY);
            // Map to requested coordinate system: bottom-left origin with offsets
            // bottom-left (imageX=0,imageY=naturalHeight) -> mappedX=57, mappedY=-2
            mappedX = imageX + 57;
            mappedY = imageY - imageEl.naturalHeight - 2;
          } else {
            // fallback to displayed pixels if natural size not available
            imageX = centerImageXDisplayed;
            imageY = centerImageYDisplayed;
            mappedX = imageX + 57;
            mappedY = imageY - imageRect.height - 2;
          }
        }

        // store mapped image coordinates in React state
        setCoords({ imageX, imageY, mappedX, mappedY });
      }
      setContextMenu({
        visible: true,
        x,
  ...
  const handleHideContextMenu = () => {
    ...
    // reset coords
    setCoords({ imageX: 0, imageY: 0, mappedX: 0, mappedY: 0 });
  };
  ...
  const stats = {
    ...
    <div className="topBar">
          ...
          {contextMenu.visible && (
            ...
            >
              Image mapped: X: <span id="coordImageX">{coords.mappedX}</span> | Y: <span id="coordImageY">{coords.mappedY}</span>
